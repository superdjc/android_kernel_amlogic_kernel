...
int aml_nftl_initialize(struct aml_nftl_blk_t *aml_nftl_blk,int no)
{
	...
	ret = aml_nftl_start((void*)aml_nftl_blk,&aml_nftl_blk->nftl_cfg,&aml_nftl_blk->aml_nftl_part,mtd->size,mtd->erasesize,mtd->writesize,mtd->oobavail,(char *)mtd->name,no,0,aml_nftl_blk->init_flag);
	if(ret != 0)
	{
    ...
    if(memcmp(mtd->name, "NFTL_Part", 9)==0)
    {
    	...
    	strcpy((char *)aml_nftl_blk->debug.name, (char*)AML_NFTL_MAGIC);
	...
	if(memcmp(mtd->name, "userdata", 8)==0)
    {
    	...
    	strcpy((char *)aml_nftl_blk->debug.name, (char*)AML_USERDATA_MAGIC);
    	aml_nftl_blk->debug.class_attrs = nftl_class_attrs;
   		error = aml_class_register(&aml_nftl_blk->debug);
		if(error)
			printk(" class register nand_class fail!\n");
